...
public:
	...
	bool SetPixelShader(std::string filename) {
		...
		if (m_pshashmap.count(filename) == 0)
		{
			m_pshashmap[filename];
			// ピクセルシェーダーオブジェクトを生成
	...
	bool SetVertexShader(std::string filename,
		...
		if (FAILED(hr))
		{
			...
			MessageBox(nullptr, "CreateVertex Shader error", "error", MB_OK);
			return false;
		}

		if (m_vshashmap.count(filename) == 0)
		{
			m_vshashmap[filename];
			m_layouthashmap[filename];
